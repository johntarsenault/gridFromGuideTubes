function [AP_step LM_step] = findMean_AP_step(gridpos_same,gridpos,meanPoint_AP_LR)

%get all possible combinations of gridpositions on same AP line 
...
for i = 1:num_transSlice
    for j = 1:size(gridpos_combos,1)
        ...
        if ~isnan(meanPoint_AP_LR(i,gridpos_combos(j,1),1)) && ~isnan(meanPoint_AP_LR(i,gridpos_combos(j,2),1))
            ...
            gridDist     = [gridDist     grid_AvsB_positionNameAP(j)];
        end
